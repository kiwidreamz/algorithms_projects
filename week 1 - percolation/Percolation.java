...
public class Percolation {
    ...
    public void open(int row, int col) {
        ...
        if (row == 1) {
            qf.union(getQuickFindIndex(row, col), SOURCE);
        }
        if (row == size) {
            qf.union(getQuickFindIndex(row, col), sink);
        }
    ...
    public boolean isFull(int row, int col) {
        if ((row > 0 && row <= size) && (col > 0 && col <= size)) {
            return qf.find(SOURCE) == qf.find(getQuickFindIndex(row, col));
        }
    ...
    public boolean percolates() {
        return qf.find(SOURCE) == qf.find(sink);
    }
